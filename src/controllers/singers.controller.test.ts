...
import { type SingersSqlRepo } from '../repositories/singers.sql.repo';
...
describe('Given an instance of the class SingersController', () => {
  const repo = {
    readAll: jest.fn(),
    readById: jest.fn(),
    create: jest.fn(),
  ...
  const controller = new SingersController(repo);
  test('Then it should be instance of the class', () => {
    expect(controller).toBeInstanceOf(SingersController);
  });
  ...
  describe('When we use the method create', () => {
    test('Then it should call repo.create', async () => {
      const singer = { name: 'x', band: 'y', age: 1000 };
      const validateSinger = singer;
      req.body = singer;
      (repo.create as jest.Mock).mockResolvedValue(singer);
      await controller.create(req, res, next);
      expect(repo.create).toHaveBeenCalledWith(validateSinger);
      expect(res.status).toHaveBeenCalledWith(201);
      expect(res.json).toHaveBeenCalledWith(singer);
    });
  });

  describe('When we use the method create and the repo throws an ERROR', () => {
    test('Then it should call repo.create and next', async () => {
      const singer = { name: 'x', band: 'y' };
      req.body = singer;
      await controller.create(req, res, next);
      expect(next).toHaveBeenCalledWith(
        new HttpError(406, 'Not Acceptable', '"age" is required')
      );
    });
  });
  ...
  describe('When we use the method create and the repo throws an ERROR', () => {
    test('Then it should call repo.create and next', async () => {
      const error = new Error('Something went wrong');
      req.body = { name: 'x', band: 'y', age: 1000 };
  ...
  describe('When we use the method update', () => {
    test('Then it should call repo.update', async () => {
      ...
      req.params = { id: '1' };
      req.body = singer;
      (repo.update as jest.Mock).mockResolvedValue(singer);
      await controller.update(req, res, next);
      expect(repo.update).toHaveBeenCalledWith('1', singer);
      expect(res.json).toHaveBeenCalledWith(singer);
    });
  });
